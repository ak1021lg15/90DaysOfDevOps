...
- **Example**:
    ```bash
    echo $NAME # Prints Ashish
    ```
- **Quoting**:
 - **Double quotes " "**: Variable is replaced with its value.
    ```bash
    echo "Hello $NAME"   # Prints Hello Ashish
    ```
...
- $?: Last command status
 ...
 ```bash
    ...
    ls /Ashish                              # This directory does not exist
    ...
    ./script.sh Ashish                      # $1 = Ashish, $# = 1, $@ = Ashish
...
- **Syntax**:
    ...
    ```
- **Example**: Prints user IDs for root,ubuntu,and Ashish
    ```bash
    for USER in root ubuntu Ashish; do
        id "$USER"
    done
    ```
